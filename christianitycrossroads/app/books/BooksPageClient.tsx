...
export default function BooksPageClient() {
  ...
  const [paidBookIds, setPaidBookIds] = useState<Set<string>>(() => new Set<string>())
  // UI state
  const [searchQuery, setSearchQuery] = useState('')
  const debouncedSearch = useDebounce(searchQuery, 300)
  const [selectedGenre, setSelectedGenre] = useState<string>('All')
  const [sortBy, setSortBy] = useState<
    'relevance' | 'price-low' | 'price-high' | 'rating' | 'newest'
  >('relevance')
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 relative">
          ...
          >
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-base sm:text-lg max-w-2xl leading-relaxed">
              Explore our curated collection of Christian books, devotionals, and
              theological works crafted for your spiritual journey.
            </p>
            ...
            {!loading && books.length > 0 && (
              ...
              >
                <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800/80 rounded-lg border border-zinc-200 dark:border-zinc-700">
                  <Library className="w-3.5 h-3.5 text-zinc-500" aria-hidden="true" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative flex-1 group">
              <Search
                className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400 group-focus-within:text-amber-500 transition-colors"
                aria-hidden="true"
              />
              <input
                ref={searchInputRef}
              ...
              {searchQuery && (
                ...
                >
                  <X className="w-3.5 h-3.5 text-zinc-400" aria-hidden="true" />
                </button>
              )}
            </div>
            ...
            <div className="relative">
              ...
              <div className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none">
                <ChevronRight
                  className="w-3.5 h-3.5 text-zinc-400 rotate-90"
                  aria-hidden="true"
                />
              </div>
            </div>
        ...
        {!loading && error && (
          ...
          >
            <div className="w-20 h-20 bg-red-50 dark:bg-red-900/10 rounded-2xl flex items-center justify-center mb-5 ring-1 ring-red-100 dark:ring-red-900/20">
              <BookOpen className="w-10 h-10 text-red-500" aria-hidden="true" />
            </div>
        ...
        {!loading && !error && filteredBooks.length === 0 && (
          ...
          >
            <div className="w-24 h-24 bg-zinc-100 dark:bg-zinc-800/50 rounded-2xl flex items-center justify-center mb-5 ring-1 ring-zinc-200 dark:ring-zinc-700">
              <Search className="w-10 h-10 text-zinc-400" aria-hidden="true" />
            </div>
