...
export default function BookDetailsPage() {
  ...
  // Verify ownership from backend (more secure than just localStorage)
  const verifyOwnership = async () => {
    try {
      ...
      if (!token) {
        // Check localStorage as fallback for non-logged in (dev mode)
    ...
    try {
      ...
      if (data.book) {
        setBook({
          ...
          fileUrl: data.book.fileUrl || '', // Backend should only send this if owned/free
  ...
  // Secure download - fetches from backend with auth, forces file download
  const handleSecureDownload = async () => {
    ...
    try {
      ...
      // Step 1: Verify ownership and get secure download URL
      ...
      // Step 2: Fetch the actual file as blob (forces download, not open)
      ...
      // Step 3: Trigger download with proper filename
  ...
  // Read online - opens in browser viewer or iframe
  const handleReadOnline = async () => {
    ...
    try {
      ...
      // Option 1: Open in new tab with Google Docs viewer (for PDFs)
      if (data.fileUrl.endsWith('.pdf')) {
        ...
        // Option 2: Direct open (will use browser's default viewer)
  ...
  const handlePaymentSuccess = async () => {
    ...
    // Refresh book details to get fileUrl
